...
ではこの節を丸ごと省略する。それ以外の line では以下の通り:
...
<!-- 末尾節はテーマの種類で分岐する。repo-backed line (テーマ JSON の repos が非空)
     は「この repo への寄与」。自由探索 line (mode: explore) は、config.toml の track に
     `report_variant = "maker"` があれば「開発アイデア」、なければ「研究ラインへの接続可能性」。 -->
...
## 開発アイデア
（maker variant の自由探索 line のみ — 「この repo への寄与」「研究ラインへの接続可能性」の
...
## 研究ラインへの接続可能性
（maker variant でない自由探索 line のみ — 上の「この repo への寄与」の代わりにこの節を置く。
