...
2. **自由探索 line** (`repos` が空、`mode: "explore"`): line の focus に沿って既存領域の
   ...
   AI 親和プラットフォームの digest、または将来の研究ラインへの接続可能性を示すこと

## 重要な制約

- **Bash は許可されていない**。スクリプト (coverage-report.sh / dr_pipeline.py) の実行や
  python3 での JSON 検証を試みない — ファイルの確認・検証はすべて Read / Grep で行う。
  ツール実行許可をユーザーに求めて終了しない（非対話実行なので誰も応答できない）
- 日本語で全て出力すること
...
`platform_digest` variant では、深掘り質問の代わりに **横断スイープを計画する**: テーマ JSON の
topic（探索アングル）に沿って、config.toml の sources を広く回って候補プラットフォームを
できるだけ多く収集し、その後「AI 親和性」「実活動」「参加入口」で 3〜7 件に絞り込む。
1 件を深掘りする記事型リサーチはしない。
...
- **digest 構成** (`platform_digest` variant のみ): 記事型の節構成（背景 / 現在の状況 / 注目プレイヤー / 未解決の問い）を使わず、テンプレート末尾の **platform_digest 用構成**に従う。3〜7 件のプラットフォームを列挙し、各件に要約・AI 親和性・実活動の証拠・参加入口 URL・一言評価 (`PILOT | WATCH | DROP`) を付ける。**実活動の証拠（直近の投稿・イベント・更新・参加者など、日付付き）を確認できない候補は掲載しない**。past_topics.json および過去の digest に掲載済みのプラットフォームは再掲禁止（大きな新展開がある場合のみ例外とし、何が新しいかを明記する）
...
- **反証・緊張節**: **maker / platform_digest variant line（config.toml の track に `report_variant = "maker"` または `"platform_digest"` がある自由探索 line）ではこの節を出力しない**。それ以外の line では: 補強材料だけを集めない。repo-backed line ではリサーチ中に repo concept と緊張・矛盾する知見を最低 1 回は意識的に探索する。見つからなければ「反証的知見は見つからなかった」と 1 文で明記する（無理にひねり出さない）。frontier モード（テーマ JSON の `mode: "frontier"`）ではこの節が本体になる — `challenges` に挙げた concept との緊張を具体的に記述する
...
- **総評節** (`platform_digest` variant の自由探索 line のみ): digest 末尾に `## 総評` 節を設ける（テンプレート参照）。件を横断して見えた傾向、`PILOT` を付けた件で今試せる最小 pilot（外部への投稿・登録はこのレポートでは実行しない）を 3-5 文の散文で書く
- **研究ラインへの接続可能性節** (`maker` / `platform_digest` 以外の自由探索 line のみ): `## この repo への寄与` の代わりに `## 研究ラインへの接続可能性` 節を設ける（テンプレート参照）。この発見が既存のrepo-backed lineの concept に将来接続しうるかを 2-3 文で書く。接続が見えなければ「純粋セレンディピティとして記録する」と明記してよい — 無理な接続をひねり出さない
...
2. past_topics.json を更新:
   - Read で現在の内容を読み込み
   - 新しいエントリを追加（テーマ数分）
   - `platform_digest` variant の line は、エントリの topic に探索アングル名に加えて
     **掲載した全プラットフォーム名を列挙**する（例: `"AI-native gallery 探索 (掲載: Foo, Bar, Baz)"`)。
     後続の digest がこのリストで再掲を dedup する
   - Write で書き戻し
