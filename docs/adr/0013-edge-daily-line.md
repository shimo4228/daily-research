...
- **観測ポイント追記 (2026-08-20)**: 初日の 2 run はいずれも主役事例の発表日が 3〜5 ヶ月前だった (在庫消化フェーズ)。運用者は鮮度制限の追加 (候補: 主役事例は「発表または浮上が 7 日以内」/ 24h 厳格版) を**保留**し、現行 focus のまま様子見と決定。`/dr-review` で主役事例の発表日・浮上日の分布を見て判断する — 在庫が掃けた後も古い事例が主役であり続けるなら制限を入れる。
